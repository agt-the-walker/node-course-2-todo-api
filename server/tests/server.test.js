...
describe('GET /todos/:id', () => {
  it('should return todo doc', done => {
    ...
    .set('x-auth', users[0].tokens[0].token).expect(200).expect(res => {
...
describe('DELETE /todos/:id', () => {
  it('should remove a todo', done => {
    ...
    .set('x-auth', users[1].tokens[0].token).expect(200).expect(res => {
  ...
  it('should not remove a todo created by another user', done => {
    ...
    request(app).delete(`/todos/${hexId}`)
    .set('x-auth', users[1].tokens[0].token).expect(404).end((err, res) => {
      if (err)
        return done(err)
...
describe('PATCH /todos/:id', () => {
  ...
  it('should not update a todo created by another user', done => {
    ...
    request(app).patch(`/todos/${hexId}`)
    .set('x-auth', users[1].tokens[0].token).send({
      text,
      completed: true
    }).expect(404).end(done)
  })
...
describe('DELETE /users/me/token', () => {
  it('should remove auth token on logout', done => {
    ...
    .set('x-auth', users[0].tokens[0].token).expect(200).end((err, res) => {
      if (err)
        return done(err)
